...
export async function insertContact(req, res){    
    ...
    if(req.files && contact.name !== '' && contact.phoneDDD !== '' && contact.phoneNumber !== '' && contact.email !== '' ){
        ...
        img.mv('./src/uploads/'+fileName, (err) => {
            if (err) {
                res.send(`<script>alert("falha ao enviar o cadastro"); window.location.href = "https://app-agenda-frontend.vercel.app/";</script>`)  
            } else {
                openDb().then(db => {
                ...
                res.send(`<script>alert("cadastro enviado com sucesso"); window.location.href = "https://app-agenda-frontend.vercel.app/";</script>`)
            }
        })
        
    } else {
        res.send(`<script>alert("todos os campos do cadastro devem ser preenchidos"); window.location.href = "https://app-agenda-frontend.vercel.app/";</script>`) 
    }
}
